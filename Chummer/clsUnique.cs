...
namespace Chummer
{
	...
	public class Attribute
	{
		...
		public void AssignLimits(string strMin, string strMax, string strAug)
		{
            ...
            if (_objCharacter.Options.StartWithAverageAttributes && this._strAbbrev != "MAG" && this._strAbbrev != "RES" && this._strAbbrev != "EDG" && this._strAbbrev != "ESS" ) {
                MetatypeMinimum += 2;
            }
		...
		private int CalculatedBP()
		{
			...
			if (_strAbbrev != "EDG" && _strAbbrev != "MAG" && _strAbbrev != "RES")
			{
				if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
				{
					...
					if (_objCharacter.GetAttribute(_strAbbrev).Value == _objCharacter.GetAttribute(_strAbbrev).TotalMaximum && _objCharacter.GetAttribute(_strAbbrev).TotalMaximum > 0)
					{
						bool blnAddPoints = true;
						// This is skipped if the Metatype's minimum and maximum are the same.
						if (_objCharacter.GetAttribute(_strAbbrev).TotalMinimum == _objCharacter.GetAttribute(_strAbbrev).TotalMaximum)
							blnAddPoints = false;

						if (blnAddPoints)
							intBP += _objCharacter.Options.BPAttributeMax;
					}
				}
				else
				{
			...
			else
			{
				...
				if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
				{
					...
					if ((_objCharacter.GetAttribute(_strAbbrev).TotalMinimum == 0 && _objCharacter.GetAttribute(_strAbbrev).TotalMaximum == 0) || _objCharacter.GetAttribute(_strAbbrev).Value == 0)
					{
						intBP += 0;
					}
					else
					{
						// Each Attribute point costs 10 BP (the first point is free).
						intBP += (_objCharacter.GetAttribute(_strAbbrev).Value + intUseEssenceLoss - _objCharacter.GetAttribute(_strAbbrev).TotalMinimum) * _objCharacter.Options.BPAttribute;
						// Having an Attribute at its racial maximum costs 25 BP instead of 10 (+15 BP total).
						// Attributes with a maximum of 0 are inactive and should not impact the BP.
						if (_objCharacter.GetAttribute(_strAbbrev).Value == _objCharacter.GetAttribute(_strAbbrev).TotalMaximum && _objCharacter.GetAttribute(_strAbbrev).TotalMaximum > 0)
						{
		...
		private int CalculatedBP()
		{
			...
			if (_intRating > 0 && !_blnIsGrouped)
			{
				if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
				{
					...
					{
						intBP += (_intRating * _objCharacter.Options.BPActiveSkill) * 2;
						// BP cost is doubled when increasing a Skill's Rating above 6.
						if (_intRating > 6)
							intBP += (_intRating - 6) * (_objCharacter.Options.BPActiveSkill * 2);
					}
					else
					{
						intBP += _intRating * _objCharacter.Options.BPActiveSkill;
						// BP cost is doubled when increasing a Skill's Rating above 6.
						if (_intRating > 6)
							intBP += (_intRating - 6) * _objCharacter.Options.BPActiveSkill;
					}
				}
				else
				{
	...
	public class Power
	{
		...
		public decimal PowerPoints
		{
			get
			{
				decimal decReturn = _intRating * PointsPerLevel;
				decReturn *= Discount;
	...
	public class MartialArtAdvantage
	{
		...
		public string DisplayNameShort
		{
			get
			{
				...
				{
					XmlDocument objXmlDocument = XmlManager.Instance.Load("martialarts.xml");
					XmlNode objNode = objXmlDocument.SelectSingleNode("/chummer/martialarts/martialart/advantages/advantage[. = \"" + _strName + "\"]");
					if (objNode != null)
					{
						if (objNode.Attributes["translate"] != null)
							strReturn = objNode.Attributes["translate"].InnerText;
