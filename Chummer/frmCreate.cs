	...
	public partial class frmCreate : Form
	{
		...
		private int CalculateBP()
		{
			...
			foreach (ContactControl objContactControl in panContacts.Controls)
			{
				...
				{
                    int calcConnectionRating = objContactControl.ConnectionRating + objContactControl.GroupRating;
                    int contactPoints = calcConnectionRating + objContactControl.LoyaltyRating;
                    if (_objOptions.ContactCostUsesMultiply)
                    {
                        contactPoints = calcConnectionRating * objContactControl.LoyaltyRating;
                    }

					if (_objCharacter.BuildMethod == CharacterBuildMethod.BP)
					{
						// The Contact's BP cost = their Connection + Loyalty Rating.
						intPointsRemain -= contactPoints * _objOptions.BPContact;
                        intPointsUsed += contactPoints * _objOptions.BPContact;
					}
					else
					{
						// The Contact's Karma cost = their (Connection + Loyalty Rating ) x Karma multiplier.
                        intPointsRemain -= contactPoints * _objOptions.KarmaContact;
                        intPointsUsed += contactPoints * _objOptions.KarmaContact;
					}
				}
			}
